    ...
    return (
        ...
        <div className="absolute top-[14%] w-[20%] h-[82%] bg-sky-custom1/65 rounded-lg left-7%">
            ...
            <button onClick={change_block} className="absolute top-17% hover:shadow-l w-40% left-5% hover:bg-sky-custom/70 rounded-xl">
              <h2 className="absolute text-[0.9vw] font-luckiest text-white top-33% left-29%">Blocked</h2>
              <img src='images/chat/block-list.png' alt='button-block'></img>
            </button>
            <button className="absolute top-17% hover:shadow-l left-55% w-40% hover:bg-sky-custom/70 rounded-xl">
              <h2 className=" absolute text-[0.9vw] font-luckiest text-white top-33% left-22%">messages</h2>
              <img src='images/chat/block-list.png' alt='button-block'></img>
            </button>
...
function Get_blocked() {
  ...
  useEffect(() => {
    fetch('https://randomuser.me/api/?results=5')
      .then(res => res.json())
      .then(data => setBlock(data.results));
  }, []);

  return (
    <div>
      {block.map((user, index) => (
        <button 
          key={index}
